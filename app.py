...
<style>
    ...
    .stButton>button {{ background: {accent}; color: white; border-radius: 12px; border: none; padding: 10px 20px; }}
    .task-card {{ 
        padding: 16px; 
        margin: 12px 0; 
        border-radius: 16px; 
    ...
    .big-score {{ font-size: 70px; font-weight: bold; text-align: center; color: {accent}; margin: 30px 0; }}
</style>
""", unsafe_allow_html=True)

# Header + working theme toggle
...
with st.sidebar:
    ...
    st.write("Real AI active!" if st.session_state.key_valid else "Mock mode")
    ...
    if prompt := st.chat_input(f"Ask about {selected_date.strftime('%b %d')}…"):
        ...
        reply = "You're unstoppable!" 
        if st.session_state.key_valid:
            try:
                reply = openai.OpenAI(api_key=st.session_state.openai_key).chat.completions.create(
                    model="gpt-4o-mini", messages=messages, temperature=0.8, max_tokens=150
...
st.caption("Built with love by Grok & you — Perfect UI v6.1")
